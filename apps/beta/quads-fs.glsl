...
void main() {
  ...
  float intensity = u_edges * exp2(-0.99 * d * d);
  ...
  float u0 =  texelFetch(field, 4 * id    ).r;
  float u1 =  texelFetch(field, 4 * id + 1).r;
  float u2 =  texelFetch(field, 4 * id + 2).r;
  float u3 =  texelFetch(field, 4 * id + 3).r;

  float s = v_Barycentric[0];
  float t = v_Barycentric[1];

  float u = (1.0 - s) * (1.0 - t) * u0 +
            (      s) * (1.0 - t) * u1 +
            (      s) * (      t) * u2 +
            (1.0 - s) * (      t) * u3;
  get_color(u, color, u_field_mode);
  ...
  vec2 uv0 = texelFetch(texcoord, 4 * id    ).xy;
  vec2 uv1 = texelFetch(texcoord, 4 * id + 1).xy;
  vec2 uv2 = texelFetch(texcoord, 4 * id + 2).xy;
  vec2 uv3 = texelFetch(texcoord, 4 * id + 3).xy;

  vec2 uv = (1.0 - s) * (1.0 - t) * uv0 +
            (      s) * (1.0 - t) * uv1 +
            (      s) * (      t) * uv2 +
            (1.0 - s) * (      t) * uv3;
  ...
  if (u_picking == id) fragColor = vec4(0.2, 0.5, 1.0, 1.0);
}
